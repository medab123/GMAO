<tbody id="tbody">
    @foreach ($demandes as $demande)
        <tr class=" {{ ($demande->status == 0) ? "primary":($demande->status == 1 ? "warning":"success") }}">
            <td>{{ $demande->id }}</td>
            <td class="text-start">
                <a href="{{ url("/demandes/$demande->id/edit") }}" class="btn btn-sm p-0"><i class="fas fa-folder-open text-primary"></i></a>
            ...
            <td><span class="badge badge-{{ ($demande->status == 0) ? "primary":($demande->status == 1 ? "warning":"success") }}">
